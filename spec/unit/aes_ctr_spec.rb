...
RSpec.describe Paygate::AesCtr do
  ...
  describe ".encrypt" do
    ...
    it "returns an encrypted text string" do
      expect(Paygate::AesCtr.encrypt(plaintext, password, num_bits)).to eq(encrypted_text)
    end

    it "returns an empty string if num_bits is invalid" do
      expect(Paygate::AesCtr.encrypt(plaintext, password, invalid_num_bits)).to eq('')
    end
  end

  describe ".decrypt" do
    it "returns a decrypted text string" do
      decrypted_text = Paygate::AesCtr.decrypt(encrypted_text, password, num_bits)
      expect(decrypted_text).to eq(plaintext)
    end

    it "returns an empty string if num_bits is invalid" do
      expect(Paygate::AesCtr.decrypt(encrypted_text, password, invalid_num_bits)).to eq('')
    end
  end
end
